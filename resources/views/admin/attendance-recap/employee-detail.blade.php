        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                @php
                    ...
                    $rowBg = $isToday ? 'bg-indigo-50/40' : ($row['status'] === 'absent' ? 'bg-red-50/30' : ($row['status'] === 'holiday' ? 'bg-pink-50/30' : ''));
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-all {{ $rowBg }} {{ $row['attendance'] ? 'cursor-pointer' : '' }}" {{ $row['attendance'] ? 'onclick=openDetail(' . $row['attendance']->id . ')' : '' }}>
                    ...
                    <td class="px-3 py-2.5 text-center">
                        @if($row['attendance'] && $row['attendance']->clock_in)
                            <span class="text-[13px] font-bold text-gray-800">{{ substr($row['attendance']->clock_in, 0, 5) }}</span>
                        @else
                            <span class="text-[10px] text-gray-300">-</span>
                        @endif
                    </td>
                    <td class="px-3 py-2.5 text-center">
                        @if($row['attendance'] && $row['attendance']->clock_out)
                            <span class="text-[13px] font-bold text-gray-800">{{ substr($row['attendance']->clock_out, 0, 5) }}</span>
                        @else
                            <span class="text-[10px] text-gray-300">-</span>
                        @endif
                    </td>
                    <td class="px-3 py-2.5 text-center">
                        @if($row['attendance'] && $row['attendance']->clock_in_photo)
