            ...
            @foreach($roles as $role)
                @php
                    ...
                    $roleIcon = match ($role) {
                        ...
                        'admin' => 'shield_person',
                        default => 'supervisor_account',
                    };
                @endphp
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                    <div class="p-5">
                        <div class="flex items-start justify-between gap-3">
                            <div class="w-10 h-10 rounded-lg {{ $role === 'superadmin' ? 'bg-red-50 text-red-600' : ($role === 'admin' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600') }} flex items-center justify-center shrink-0">
                        ...
                        <h2 class="mt-4 text-[16px] font-bold text-gray-900">{{ ucfirst($role) }}</h2>
                        <p class="mt-1 text-[12px] leading-5 text-gray-500">
                            {{ $role === 'superadmin' ? 'Selalu punya semua akses dan tidak perlu diubah.' : 'Permission default untuk role ' . ucfirst($role) . '.' }}
                        </p>
                    </div>
                ...
                <div class="p-5 space-y-4">
                        ...
                        <select name="employee_id" onchange="this.form.submit()" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-[13px] bg-white">
                            ...
                            @foreach($admins as $adminUser)
                                <option value="{{ $adminUser->id }}" {{ $selectedEmployee?->id === $adminUser->id ? 'selected' : '' }}>
                                    {{ $adminUser->full_name }} ({{ ucfirst($adminUser->role) }})
                                </option>
                            @endforeach
                        </select>
                    </form>
                    ...
                    @if($selectedEmployee)
                        ...
                        <div class="rounded-lg border border-gray-200 p-4">
                            ...
                            <div class="text-[12px] text-gray-500 mt-0.5">Role: {{ ucfirst($selectedEmployee->role) }}</div>
...
<div id="rolePermissionModal" class="hidden fixed inset-0 items-center justify-center px-4 py-6" style="z-index: 1000;">
    <div class="absolute inset-0 bg-slate-900/45 backdrop-blur-[2px]" data-role-modal-close></div>
    <div class="role-permission-dialog relative w-full max-w-5xl bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col" style="max-height: calc(100vh - 48px); z-index: 1;">
        ...
        <div class="overflow-y-auto" style="max-height: calc(100vh - 136px);">
            @foreach($roles as $role)
                    ...
                    <div class="p-5 space-y-5">
                        @foreach($groups as $groupLabel => $items)
                            ...
                            <section class="rounded-lg border border-gray-200 overflow-hidden">
                                ...
                                <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-2">
...
@if($selectedEmployee)
<div id="employeeOverrideModal" class="hidden fixed inset-0 items-center justify-center px-4 py-6" style="z-index: 1000;">
    <div class="absolute inset-0 bg-slate-900/45 backdrop-blur-[2px]" data-override-modal-close></div>
    <form method="POST" action="{{ route('admin.role-permissions.employees.update', $selectedEmployee) }}" class="role-permission-dialog relative w-full max-w-5xl bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col" style="max-height: calc(100vh - 48px); z-index: 1;">
        @csrf
        @method('PUT')
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <div>
                <h3 class="text-[16px] font-bold text-gray-900">Atur Override Admin</h3>
                <p class="text-[12px] text-gray-500 mt-0.5">{{ $selectedEmployee->full_name }} - Role {{ ucfirst($selectedEmployee->role) }}</p>
            </div>
        ...
        <div class="p-5 space-y-5 overflow-y-auto" style="max-height: calc(100vh - 150px);">
            @foreach($groups as $groupLabel => $items)
                <section class="rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4 space-y-2">
                        @foreach($items as $permission => $label)
                            @php
                                ...
                                $baseAllowed = !empty($roleStates[$selectedEmployee->role][$permission]);
                            @endphp
